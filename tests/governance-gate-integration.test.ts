...
describe('post handler governance gate integration', () => {
  ...
  it('inserts posts that pass the governance gate (matching on-topic content)', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/1', 'did:plc:abc', 'cid123', {
      text: 'Building on atproto is awesome',
  ...
  it('rejects posts that fail the governance gate (no topic match)', async () => {
    ...
    await handlePost('at://did:plc:abc/app.bsky.feed.post/2', 'did:plc:abc', 'cid456', {
      text: 'Random unrelated post about lunch',
      createdAt: new Date().toISOString(),
    });
  ...
  it('governance gate runs AFTER keyword content filter (both must pass)', async () => {
    ...
    await handlePost('at://did:plc:abc/app.bsky.feed.post/6', 'did:plc:abc', 'cidghi', {
      text: 'This post does not match include keywords',
      createdAt: new Date().toISOString(),
    });
...
describe('post handler media-without-text gate', () => {
  ...
  it('rejects image-only posts with no text', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/img1', 'did:plc:abc', 'cidimg1', {
      createdAt: new Date().toISOString(),
  ...
  it('rejects image posts with text shorter than INGESTION_MIN_TEXT_FOR_MEDIA', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/img2', 'did:plc:abc', 'cidimg2', {
  ...
  it('inserts image posts with sufficient text', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/img3', 'did:plc:abc', 'cidimg3', {
  ...
  it('inserts text-only posts regardless of text length', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/txt1', 'did:plc:abc', 'cidtxt1', {
  ...
  it('media gate runs BEFORE topic classification (saves CPU)', async () => {
    ...
    await handlePost('at://did:plc:abc/app.bsky.feed.post/img4', 'did:plc:abc', 'cidimg4', {
      createdAt: new Date().toISOString(),
      embed: { images: [{ alt: '', image: {} }] },
    });
...
describe('post handler AT Protocol label filtering', () => {
  ...
  it('rejects posts with porn label', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/nsfw1', 'did:plc:abc', 'cidnsfw1', {
      text: 'Some text',
      createdAt: new Date().toISOString(),
  ...
  it('rejects posts with sexual label', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/nsfw2', 'did:plc:abc', 'cidnsfw2', {
      text: 'Some text',
      createdAt: new Date().toISOString(),
  ...
  it('rejects posts with graphic-media label', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/nsfw3', 'did:plc:abc', 'cidnsfw3', {
      text: 'Some text',
      createdAt: new Date().toISOString(),
  ...
  it('rejects posts with nudity label', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/nsfw4', 'did:plc:abc', 'cidnsfw4', {
      text: 'Some text',
      createdAt: new Date().toISOString(),
  ...
  it('inserts posts with no labels', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/clean1', 'did:plc:abc', 'cidclean1', {
