...
describe('post handler AT Protocol NSFW label filtering', () => {
  ...
  it('filters posts with porn content label', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/1', 'did:plc:abc', 'cid1', {
      text: 'Some post',
  ...
  it('filters posts with nudity content label', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/2', 'did:plc:abc', 'cid2', {
      text: 'Another post',
      createdAt: new Date().toISOString(),
  ...
  it('allows posts with non-NSFW custom labels', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/3', 'did:plc:abc', 'cid3', {
  ...
  it('allows posts with no labels field', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/4', 'did:plc:abc', 'cid4', {
  ...
  it('allows posts with empty labels array', async () => {
    await handlePost('at://did:plc:abc/app.bsky.feed.post/5', 'did:plc:abc', 'cid5', {
  ...
  it('allows NSFW-labeled posts when FILTER_NSFW_LABELS is disabled', async () => {
    ...
    await handlePost('at://did:plc:abc/app.bsky.feed.post/6', 'did:plc:abc', 'cid6', {
      text: 'Labeled post',
      createdAt: new Date().toISOString(),
      labels: { values: [{ val: 'porn' }] },
    });
