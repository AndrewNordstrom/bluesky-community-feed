...
export function registerAuthRoute(app: FastifyInstance): void {
      ...
      try {
        ...
        if (!session) {
          return reply.code(401).send({
            ...
            message:
              'Invalid handle or app password. Make sure you are using an app password from Bluesky settings.',
          });
        }
   ...
   */
  app.get(
    '/api/governance/auth/session',
    {
      schema: {
        tags: ['Auth'],
        summary: 'Get current session',
        description:
          "Returns the authenticated user's session info. Requires a valid session cookie or bearer token.",
