...
export function registerEpochsRoute(app: FastifyInstance): void {
  ...
  app.get(
    '/api/governance/epochs',
    {
      schema: {
        ...
        summary: 'List epochs',
        description:
          'Returns a paginated list of governance epochs with weights, vote counts, and content rules. Optionally filter by status.',
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const epochs = await Promise.all(
        result.rows.map(async (row) => {
          ...
          const contentRules = toContentRules(
            (row.content_rules as ContentRulesRow | null) ?? null,
          );
          ...
          return {
            ...
            status: epoch.status,
            phase:
              (row.phase as string | null) ?? (epoch.status === 'voting' ? 'voting' : 'running'),
   ...
   */
  app.get(
    '/api/governance/epochs/current',
    {
      schema: {
        tags: ['Governance'],
        summary: 'Get current epoch',
        description:
          'Returns the current active or voting epoch with weights, vote/subscriber counts, voting schedule, and content rules.',
        response: {
          200: {
            type: 'object',
            properties: {
              ...
              subscriber_count: {
                type: 'integer',
                description: 'Active subscribers (potential voters)',
              },
              created_at: { type: 'string', format: 'date-time' },
    ...
    async (_request: FastifyRequest, reply: FastifyReply) => {
      ...
      const contentRules = toContentRules(
        (result.rows[0].content_rules as ContentRulesRow | null) ?? null,
      );
      ...
      return reply.send({
        ...
        status: epoch.status,
        phase:
          (result.rows[0].phase as string | null) ??
          (epoch.status === 'voting' ? 'voting' : 'running'),
        weights: epoch.weights,
        vote_count: parseInt(voteCount.rows[0].count),
   ...
   */
  app.get(
    '/api/governance/epochs/:id',
    {
      schema: {
        tags: ['Governance'],
        summary: 'Get epoch by ID',
        description:
          'Returns full details for a specific epoch including weights, vote statistics (averages and ranges), and content rules.',
        ...
        querystring: {
          ...
          properties: {
            force: {
              type: 'boolean',
              default: false,
              description: 'Skip vote count check (for testing)',
            },
          },
        },
        response: {
