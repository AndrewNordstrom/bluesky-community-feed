...
export function registerWeightsRoute(app: FastifyInstance): void {
   ...
   */
  app.get(
    '/api/governance/weights/history',
    {
      schema: {
        tags: ['Governance'],
        summary: 'Weight history',
        description:
          'Returns all epochs with their weights, ordered newest first. Useful for timeline visualization of governance changes.',
   ...
   */
  app.get(
    '/api/governance/weights/compare',
    {
      schema: {
        tags: ['Governance'],
        summary: 'Compare epoch weights',
        description:
          'Compare weights between two epochs, showing the difference for each component.',
