  ...
  app.get(
    '/api/governance/research-consent',
    {
      schema: {
        ...
        description:
          "Returns the authenticated user's current research consent status. Research consent is separate from TOS — declining does not restrict governance access.",
  ...
  app.post(
    '/api/governance/research-consent',
    {
      schema: {
        ...
        description:
          "Record or update the user's research consent decision. Consent changes are logged to the audit trail.",
        security: governanceSecurity,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const previous = await db.query(`SELECT research_consent FROM subscribers WHERE did = $1`, [
        did,
      ]);
