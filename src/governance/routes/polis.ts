...
export function registerPolisRoute(app: FastifyInstance): void {
   ...
   */
  app.get(
    '/api/governance/polis',
    {
      schema: {
        tags: ['Governance'],
        summary: 'Polis conversation info',
        description:
          'Returns Polis conversation info if configured. Currently a placeholder for future deliberation integration.',
        response: {
          200: {
            type: 'object',
            properties: {
    ...
    async (_request: FastifyRequest, reply: FastifyReply) => {
      ...
      return reply.send({
        ...
        description:
          'Polis deliberation for feed governance. Participate in discussions about how the feed algorithm should work.',
        status: 'placeholder',
   ...
   */
  app.get(
    '/api/governance/polis/status',
    {
      schema: {
        tags: ['Governance'],
        summary: 'Polis integration status',
        description:
          'Returns the current status of the Polis integration roadmap and planned features.',
        response: {
          200: {
            type: 'object',
            properties: {
