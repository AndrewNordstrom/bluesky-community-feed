...
const VoteSchema = z
  ...
  .refine(
    (data) => {
      ...
      const hasAllWeights = GOVERNANCE_WEIGHT_VOTE_FIELDS.every(
        (field) => data[field] !== undefined,
      );

      if (!hasAllWeights) return false;

      const sum = GOVERNANCE_WEIGHT_VOTE_FIELDS.reduce(
        (acc, field) => acc + (data[field] as number),
        0,
      );
        ...
        body: {
          ...
          properties: {
            ...
            engagement: {
              ...
              minimum: 0,
              maximum: 1,
              description: 'Weight for engagement component',
            },
            bridging: {
              type: 'number',
              minimum: 0,
              maximum: 1,
              description: 'Weight for bridging component',
            },
            source_diversity: {
              type: 'number',
              minimum: 0,
              maximum: 1,
              description: 'Weight for source diversity component',
            },
            relevance: {
              type: 'number',
              minimum: 0,
            ...
            include_keywords: {
              ...
              maxItems: 20,
              description: 'Keywords to boost in feed',
            },
            exclude_keywords: {
              type: 'array',
              items: { type: 'string', maxLength: 50 },
              maxItems: 20,
              description: 'Keywords to filter from feed',
            },
            topic_weights: {
              type: 'object',
              additionalProperties: { type: 'number', minimum: 0, maximum: 1 },
              description: 'Topic slug → weight mapping',
            },
          },
        },
        response: {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (subscriber.rows.length === 0) {
        return reply.code(403).send({
          error: 'Forbidden',
          message:
            'You must be an active feed subscriber to vote. Use the feed first to become a subscriber.',
        });
      }
      ...
      if (vote.topic_weights && Object.keys(vote.topic_weights).length > 0) {
        ...
        const validSlugs = new Set(
          slugResult.rows.map((r: Record<string, unknown>) => r.slug as string),
        );
      ...
      if (hasWeights) {
        ...
        normalized = normalizeWeights(votePayloadToWeights(weightPayload));
      ...
      try {
        ...
        const message = isNewVote ? 'Your vote has been recorded.' : 'Your vote has been updated.';
          ...
          200: {
            ...
            properties: {
              ...
              voted_at: {
                ...
                format: 'date-time',
                nullable: true,
                description: 'When the vote was last cast or updated',
              },
